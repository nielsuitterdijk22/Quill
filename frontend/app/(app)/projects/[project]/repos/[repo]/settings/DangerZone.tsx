...
function VisibilityRow({
  ...
  const action = changeVisibilityAction.bind(null, project, repo);
...
function RenameRow({ project, repo }: { project: string; repo: string }) {
  const action = renameRepoAction.bind(null, project, repo);
  const [state, formAction] = useFormState(action, initial);
  return (
    <div className="danger-row">
      <div className="danger-row-text">
...
function ArchiveRow({
  ...
  const action = setRepoArchivedAction.bind(null, project, repo, !archived);
  const [state, formAction] = useFormState(action, initial);
  return (
    <div className="danger-row">
      <div className="danger-row-text">
...
// arm the button; the action redirects to the project overview on success.
function DeleteRow({ project, repo }: { project: string; repo: string }) {
  const action = deleteRepoAction.bind(null, project, repo);
  const [state, formAction] = useFormState(action, initial);
  return (
    <div className="danger-row">
      <div className="danger-row-text">
...
export function DangerZone({
  ...
  return (
    <div className="panel danger-zone">
      <VisibilityRow project={project} repo={repo} visibility={visibility} />
      <RenameRow project={project} repo={repo} />
      <ArchiveRow project={project} repo={repo} archived={archived} />
      <DeleteRow project={project} repo={repo} />
    </div>
  );
}
