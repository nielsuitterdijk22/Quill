...
// CommitsPage lists the commit log of a repository at a ref.
export default async function CommitsPage({
  params,
}: {
  params: { org: string; repo: string; ref: string };
}) {
  const token = getToken();
  if (!token) notFound();

  const ref = params.ref;
  const result = await getCommits(token, params.org, params.repo, ref, "", 50);
  if (!result.ok) {
