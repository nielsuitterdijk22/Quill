...
// createPullAction opens a pull request, then redirects to it. The org and repo
...
export async function createPullAction(
  ...
  const result = await createPull(token, org, repo, { title, body, head, base });
  if (!result.ok) return { error: result.error };

  redirect(`/orgs/${org}/${repo}/pulls/${result.data.pull.number}`);
}
