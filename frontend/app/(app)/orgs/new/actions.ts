"use server";

import { redirect } from "next/navigation";

import { createOrg } from "../../../lib/api";
import { getToken } from "../../../lib/session";

export type CreateState = { error?: string };

// slugify derives a URL-safe slug from free text, matching the backend's slug
// rules (lowercase alphanumerics plus -, _, .).
function slugify(input: string): string {
  return input
    .toLowerCase()
    .trim()
    .replace(/[^a-z0-9._-]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 63);
}

// createOrgAction provisions an organization for the signed-in user, then
// redirects to its repository list. Validation errors are returned for display.
export async function createOrgAction(
  _prev: CreateState,
  formData: FormData,
): Promise<CreateState> {
  const token = getToken();
  if (!token) return { error: "Your session has expired. Sign in again." };

  const name = String(formData.get("name") ?? "").trim();
  const slug = slugify(String(formData.get("slug") ?? "") || name);
  const description = String(formData.get("description") ?? "").trim();

  if (!name) return { error: "Enter an organization name." };
  if (!slug) return { error: "Enter a valid slug (letters, digits, - _ .)." };

  const result = await createOrg(token, { slug, name, description });
  if (!result.ok) return { error: result.error };

  redirect(`/orgs/${result.slug}`);
}
