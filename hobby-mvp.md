...
- [ ] **Rate-limit auth endpoints.** `/auth/register` and `/auth/login` have no
  brute-force protection. Add a per-IP sliding-window limiter (e.g. 10
  attempts / minute) in the chi middleware stack.
- [ ] **Harden JWT cookie flags.** Audit the cookie set in the login handler:
...
- [ ] **Security response headers.** Add a middleware pass in `server/` that sets
  `Content-Security-Policy`, `X-Frame-Options: DENY`,
  `X-Content-Type-Options: nosniff`, and `Referrer-Policy: no-referrer` on
  every response.
- [ ] **Path traversal check on `/contents`.** The content endpoint passes a
...
- [ ] **Document HTTPS requirement.** The deploy guide must warn that running
...
- [ ] **`viewerIsAuthor` flag from the backend.** The PR page currently compares
...
- [ ] **Graceful shutdown in `cmd/api` and `cmd/dispatch`.** Both entrypoints
...
- [ ] **Health check endpoints.** Add `GET /healthz` that checks Postgres
  ...
  uptime monitors.
- [ ] **Forgejo unavailable handling.** If Forgejo is down, most routes return an
...
- [ ] **Landing page for logged-out visitors.** Right now an unauthenticated
...
- [ ] **Empty state for Projects page.** When a new user has no projects, show a
  prompt ("Create your first project") with a direct link to the creation
  form, not a blank list.
- [ ] **Empty states for Repos, PRs, and Pipelines.** Each list page should have
  contextual empty-state copy rather than rendering an empty table.
- [ ] **Post-registration flow.** After registering, drop the user directly into
  project creation (or a "get started" checklist) rather than the empty
  dashboard.

### User account basics

- [ ] **Password change.** Add a "Change password" section to the settings page
  (current password + new password + confirm).
- [ ] **Forgot password / reset flow.** Minimum viable: admin-reset via a CLI
...
- [ ] **Account deletion (GDPR — see below).** Self-service "Delete my account"
  in settings: purges the Quill DB record, revokes all git tokens, and
  deletes the mirrored Forgejo account.
- [ ] **Email change.** Let users update their email address (settings page).

### Repository basics

- [ ] **Clone URL displayed prominently.** On the repo overview page, show the
  HTTP clone URL (and SSH if wired) in a copyable `<input>` at the top, like
  GitHub does. Currently it's unclear how to clone.
- [ ] **SSH key management.** Let users add their public SSH keys (stored in
...
- [ ] **Cache `/markup` renders.** Every repo tree page does a POST to Forgejo
...
- [ ] **Auto-detect README.** If the repo root contains a `README.md` (or
...
- [ ] **Account deletion (full purge).** Covered in UX above — emphasising it
...
- [ ] **Data export.** Let users download a ZIP or JSON of their own data:
...
- [ ] **No external resources in the frontend.** Audit `frontend/` for any
...
- [ ] **Cookie consent banner not required (document this).** If Quill uses only
...
- [ ] **Privacy policy template.** Add a `docs/operator-privacy-policy.md`
...
- [ ] **Document all environment variables.** Every variable in `.env.example`
...
- [ ] **Backup and restore documentation.** Document how to dump Postgres
  (`pg_dump`) and back up the Forgejo data volume, and how to restore from
  both. One page in `deploy/compose/README.md` is enough.
- [ ] **Upgrade path documentation.** When a new Quill release ships a schema
...
- [ ] **Resource requirements.** State the minimum RAM and CPU for a hobby
  instance (Forgejo + Postgres + Quill API + dispatcher). Rough guide so
  users know if a €5/month VPS is enough.

### Transparency

- [ ] **License and source in UI footer.** The footer (or about page) should
  link to the GitHub repo and display "Apache 2.0" — so users of a hosted
  instance know their rights.
- [ ] **No telemetry statement in README.** Add a one-liner confirming Quill
...
- [ ] Write a "why Quill?" section in `README.md` — two paragraphs covering
